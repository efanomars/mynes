...
def testAll(sBuildType, sInstallDir, sSudo, bBuildStatic, bSanitize):
	...
	subprocess.check_call("./scripts/uninstall_mynes-all.py -y --installdir {}  {}".format(sInstallDir, sSudo).split())

	subprocess.check_call("./scripts/install_mynes-all.py -b {}  -t On  -d On --docs-to-log  --installdir {}  {} {} {}"\
			.format(sBuildType, sInstallDir, sSudo, sStatic, sSanitize).split())


	os.chdir("libstmm-mynes/build")
	subprocess.check_call(sSanitizeOptions + " make test", shell=True)
	if os.path.getsize("libstmm-mynes_doxy.log") > 0:
		raise RuntimeError("Error: libstmm-mynes_doxy.log not empty")
	os.chdir("../..")

	os.chdir("libstmm-mynes-xml/build")
	subprocess.check_call("make test", shell=True)
	if os.path.getsize("libstmm-mynes-xml_doxy.log") > 0:
		raise RuntimeError("Error: libstmm-mynes-xml_doxy.log not empty")
	os.chdir("../..")

	os.chdir("mynes/build")
	subprocess.check_call("make test", shell=True)
	os.chdir("../..")
...
def main():
	...
	print("---------------------------")
	print("testall.py (mynes) started!")
	print("---------------------------")
	...
	print("-----------------------------------------")
	print("testall.py (mynes) finished successfully!")
	print("-----------------------------------------")
