...
# File:   uninstall_libstmm-mynes-xml.py

# Removes all files installed by the install_libstmm-mynes-xml.py script.
...
def main():
	import argparse
	oParser = argparse.ArgumentParser("Removes all files created by install_libstmm-mynes-xml.py")
	...
	if not oArgs.bNoUninstall:
		subprocess.check_call("{} rm -r -f            {}/include/stmm-mynes-xml".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f             {}/lib/libstmm-mynes-xml.so*".format(sSudo, sInstallDir), shell=True)
	...
	if not oArgs.bNoClean:
		os.chdir("..")
		subprocess.check_call("{} rm -r -f libstmm-mynes-xml/build".format(sSudo).split())
